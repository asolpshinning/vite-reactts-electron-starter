...
export default function Notes() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const file = join(await chatRoot(), 'notes', `${record?.id}.${record?.ext}`);
    ...
    const handleDelete = async () => {
        if (opData?.length === selectedRows.length) {
            const notesDir = join(await chatRoot(), 'notes');
            await fs.rmdir(notesDir, { recursive: true });
            await handleRefresh();
        ...
        const rows = selectedRows.map(async (i) => {
            const file = join(await chatRoot(), 'notes', `${i?.id}.${i?.ext}`);
            await fs.unlink(file);
            return file;
        });
    ...
    const handleRefresh = async () => {
        ipcRenderer.send('download_list', { pathname: CHAT_NOTES_JSON, dir: 'notes' });
        rowReset();
