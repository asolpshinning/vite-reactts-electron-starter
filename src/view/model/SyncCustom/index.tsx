...
export default function SyncCustom() {
    ...
    const handleSync = async (filename: string) => {
        ...
        const file = join(await chatRoot(), 'cache_model', filename);
        const filePath = await getPath(record);

        // https or http
        if (/^http/.test(record?.protocol)) {
            const data = await ipcRenderer.invoke('sync_user_prompts', { url: filePath, dataType: record?.ext });
            if (data) {
        ...
        if (isJson) {
            ...
            const list: Record<string, string>[] = await ipcRenderer.invoke('parse_prompt', { data });
