...
export default function Dashboard() {
    ...
    useInit(async () => {
        const getOS = await ipcRenderer.invoke('get-platform');
        const conf = await readJSON(APP_CONF_JSON);
        const appTheme = await ipcRenderer.invoke('get-theme');
    ...
    const handleLink = async (item: Record<string, any>) => {
        await ipcRenderer.invoke('open-window', {
    ...
    if (list?.length === 0) {
        return (
            <div className="dashboard-no-data">
                ...
                <div className="txt">
                    Go to <a onClick={() => ipcRenderer.send('control_window')}>{'Control Center -> Awesome'}</a> to add
